...
func main() {
	for _ = range time.Tick(time.Second * 30) {
		...
		for k := 0; k < len(autoGeneratedpos.Positions); k++ {
			if autoGeneratedpos.Positions[k].Symbol == tickerName {
				positionBTCindex = k
			}
		}
		...
		if len(openOrders) == 1 && startShortTo382 {
			...
			stopOrder, err := futuresClient.NewCreateOrderService().Symbol(tickerName).
		...
		if takeProfit382Condition {
			...
			takeProfitOrder, err := futuresClient.NewCreateOrderService().
				Symbol(tickerName).Side(futures.SideTypeBuy).Type(futures.OrderTypeTakeProfitMarket).
